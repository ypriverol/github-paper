...
comment on, and discuss code \cite{Dabbish:2012}. A diverse range of
...
GitHub as a productive place to share code, ideas and collaborate (see Table 1).
...
software, is tracked and controlled by an individual and unique
identifier. While git has a complex set of commands and can be used
for rather complex operations, learning and apply the basics 
...
previous code version. In larger projects, multiple users contribute to the
...
(https://www.git-scm.com/downloads/guis) also exist. Many editors and development
...
_Collaborator_, thereby granting write access.
...
the original (upstream) project (requested by opening an _pull request_) , thereby improving the original
...
`semantic versioning' practice, with the format X.Y.Z, with X being the major, Y the
...
checking data and software dependencies, and it can be automated through
GitHub.
...
new ideas, and enhancements, using a powerful but optional tagging
...
optional text (comments). If the developer needs more information, 
they can simply request it in a comment. Alternatively, it is
